...
def print_metrics(model, train_dataset, test_dataset):
    ...
    print("Acc= " + np.mean(predicted == test_dataset.target))
    ...
    text += "\n" + str(metrics.confusion_matrix(test_dataset.target, predicted))
    return text
...
class Classifier:
    def __init__(self):
        self.train_dataset, self.val_dataset, self.test_dataset = get_datasets(split=False)
